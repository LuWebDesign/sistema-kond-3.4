...
function renderCatalog() {
  ...
  catalogGrid.innerHTML = catalogProducts.map(product => {
    ...
    // Colores del badge de descuento
    let discountBadgeColor = '#10b981'; // Verde por defecto
    let discountBadgeTextColor = '#ffffff'; // Blanco por defecto
    
    if (promoResult && promoResult.discountBadgeColor) {
      discountBadgeColor = promoResult.discountBadgeColor;
      
      // Calcular color de texto si es 'auto'
      if (promoResult.discountBadgeTextColor === 'auto') {
        // Función simple de contraste
        const hex = discountBadgeColor.replace('#', '');
        const r = parseInt(hex.substr(0, 2), 16);
        const g = parseInt(hex.substr(2, 2), 16);
        const b = parseInt(hex.substr(4, 2), 16);
        const luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
        discountBadgeTextColor = luminance > 0.5 ? '#000000' : '#ffffff';
      } else if (promoResult.discountBadgeTextColor) {
        discountBadgeTextColor = promoResult.discountBadgeTextColor;
      }
    }
      ...
      <div class="catalog-product-card" data-id="${product.id}">
        <div class="catalog-product-image-container">
          ${product.imagen 
            ...
            : `<div class="catalog-product-placeholder"><span>Sin imagen</span></div>`
          }
          ${allBadges.length > 0 
            ? allBadges.map((b, idx) => `<span class="catalog-product-badge" style="background-color: ${b.color || '#ef4444'}; color: ${b.textColor || '#ffffff'}; top: ${8 + idx * 32}px;">${escapeHtml(b.text)}</span>`).join('')
            : ''
          }
          ${hasDiscount && discountPercent > 0 
            ? `<span class="catalog-product-badge" style="background-color: ${discountBadgeColor}; color: ${discountBadgeTextColor}; top: ${8 + (allBadges.length * 32)}px;">-${discountPercent}%</span>` 
            : ''
          }
        </div>
        ...
        <div class="catalog-product-info">
          ...
          ${product.medidas ? `<p class="catalog-product-measures">${escapeHtml(product.medidas)}</p>` : ''}
          ${product.categoria ? `<span class="catalog-product-type ${product.tipo || 'venta'}">${escapeHtml(product.categoria)}</span>` : ''}
          ...
          <div class="catalog-product-actions">
            <div class="catalog-quantity-selector">
              <button class="qty-btn qty-decrease" data-id="${product.id}" aria-label="Disminuir cantidad">−</button>
              <input type="number" class="qty-input" value="1" min="1" max="999" data-id="${product.id}" aria-label="Cantidad">
              <button class="qty-btn qty-increase" data-id="${product.id}" aria-label="Aumentar cantidad">+</button>
            </div>
...
function savePedidoCatalogo(pedidoData) {
  ...
  // Usar helper seguro que puede omitir comprobante si hay QuotaExceeded
  try {
    const result = savePedidosCatalogoSafely(pedidosCatalogo);
    if (!result || !result.success) {
      // Si no se guardó correctamente, intentar el fallback local previo (mantener compatibilidad)
      console.warn('savePedidosCatalogoSafely falló', result && result.error);
      throw result && result.error ? result.error : new Error('savePedidosCatalogoSafely failed');
    }
    if (result.omittedComprobante) {
    ...
    // Mantener comportamiento anterior: reintentos y limpieza final
    console.warn('localStorage.setItem failed for pedidosCatalogo (fallback)', err);
    try {
