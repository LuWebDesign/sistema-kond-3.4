...
- Trigger: Creating collapsible sections for forms, accordion components in admin pages.
- Rules:
  - Use the CollapsibleSection component for long forms (product new/edit, etc.).
  - Sections: Información Básica, Descripción, Producción, Material, Costos, Imágenes.
  - Guardado por sección sin colapsar tras guardar (mantener estado expanded).
  - Use CollapseButton and CollapseContent pattern.

### theme-centralized
- Path: C:\Dev\sistema-kond-3.4\skills\theme-centralized\SKILL.md
- Trigger: Adding dark mode/theme toggle to admin pages.
- Rules:
  - Use the centralized theme system via `document.body.getAttribute('data-theme')`.
  - Do NOT use local state like `useState('dark')` or localStorage keys like `finanzas_dark`.
  - CSS: Use CSS variables (`--bg-card`, `--text-primary`, `--border-color`, etc.) instead of hardcoded colors.
  - Flash prevention: _document.js has inline script that applies theme from localStorage before React loads.
  - To toggle: use `setAppTheme('light'|'dark')` from `utils/theme.js` or manually set body attr + localStorage.
